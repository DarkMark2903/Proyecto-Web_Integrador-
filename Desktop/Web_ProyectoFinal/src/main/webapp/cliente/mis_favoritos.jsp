...
<head>
    ...
    <style> .card-product:hover { box-shadow: 0 0.5rem 1rem rgba(0, 0, 0, 0.25) !important; transform: translateY(-5px); transition: all 0.3s ease; } .text-golden { color: #c5a059; } </style>
</head>
<body>
    <jsp:include page="/header/header.jsp" />
    <main class="container my-5">
        <div class="row"><div class="col-12 text-center"><h1 class="mb-4 fw-bold">Mis Favoritos</h1><p class="lead text-golden">Consulta los productos que te encantaron.</p><hr></div></div>
            ...
            <div class="col-lg-12">
                <div class="card shadow-lg border-0 p-4">
                    ...
                    <c:choose>
                                    ...
                                    <div class="col">
                                        <div class="card card-product h-100 text-center">
                                            <a href="${pageContext.request.contextPath}/detalle_producto?id=${p.id_producto}">
                                                ...
                                                <img src="${rutaImg}" class="card-img-top p-3" alt="${p.nombre}" style="height: 200px; object-fit: contain;" onerror="this.src='https://via.placeholder.com/200?text=Error'">
                                            </a>
                                            <div class="card-body d-flex flex-column">
                                                <h5 class="card-title fw-bold text-truncate">${p.nombre}</h5>
                                                <p class="fs-5 fw-bold text-warning">S/ ${p.precio}</p>
                                                <div class="d-grid gap-2 mt-auto">
                                                    <a href="${pageContext.request.contextPath}/eliminarFavorito?idProducto=${p.id_producto}" class="btn btn-danger btn-sm" title="Quitar"><i class="bi bi-x-circle me-2"></i> Quitar</a>
                        ...
                        <c:otherwise>
                            <div class="text-center py-5"><i class="bi bi-heart-fill display-4 text-golden"></i><h4 class="mt-3 text-golden">Aún no has agregado productos a tus favoritos.</h4><a href="${pageContext.request.contextPath}/catalogo" class="btn btn-warning mt-3">Explorar Catálogo</a></div>
                        </c:otherwise>
                    </c:choose>
