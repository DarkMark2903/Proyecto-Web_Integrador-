...
public class ProductoDAO {
    ...
    public List<Producto> obtenerTodos() {
        return listarProductosAdmin(null, 0, null);
    }
    
    // MÉTODO ACTUALIZADO: Filtrado para ADMIN (Nombre, Categoría, Orden Stock)
    public List<Producto> listarProductosAdmin(String busqueda, int idCategoria, String ordenStock) {
        ...
        if ("menor_stock".equals(ordenStock)) {
            sql.append(" ORDER BY stock ASC");
        } else if ("mayor_stock".equals(ordenStock)) {
            sql.append(" ORDER BY stock DESC");
        } else {
            sql.append(" ORDER BY id_producto DESC"); // Por defecto
        }
